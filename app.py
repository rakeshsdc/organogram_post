...
def batch_top_level_form():
    ...
    with st.form(f"l1_form_{nz}"):
        ...
        head[1].markdown("**Department**")
        for i in range(n):
            c = st.columns([3, 2])
            c[0].text_input("Position title", key=f"l1_t_{nz}_{i}", label_visibility="collapsed",
                            placeholder="e.g. Director")
            c[1].text_input("Department", key=f"l1_d_{nz}_{i}", label_visibility="collapsed")
    ...
    if save or save_more:
        ...
        if mode == "add":
            pos2, extra2, new_id = du.add_position(
                pos, extra, title=title, department=department, reports_to=parent,
            ...
            pos2, extra2 = du.update_position(
                pos, extra, pid, position_title=title, department=department,
...
def tab_enter():
    ...
    with left:
        ...
        query = top[1].text_input("Search", placeholder="search title / department",
        ...
        for pid, depth in du.tree_rows(pos):
            ...
            if depth + 1 > show_to:
                continue
            if q and q not in " ".join([r["position_title"], r["department"], pid]).lower():
                continue
            shown += 1
...
def tab_table():
    ...
    edited = st.data_editor(
        ...
        column_config={
            "name": None,  # person names are not used; column kept only so old files still load
...
def tab_chart():
    ...
    opts = ChartOptions(
        ...
        show_contact=contact, institution=ss.get("institution", "").strip(),
    )
...
def tab_checks():
    ...
    if not errors:
        ...
        with c2:
            st.markdown("**Summary**")
            vacant = int((ss.pos["status"] == "Vacant").sum())
