...
| `name` | Person in the post; blank = vacant |
| `department` | Used for colours |
...
* **Malayalam or other non-English names:** the on-screen chart uses your browser's fonts. The PDF/Word pictures are drawn on the server, which may not have a suitable font, so such names could appear as empty boxes. If so, add a suitable font package (for example `fonts-smc` for Malayalam, or `fonts-noto-core`) as a new line in `packages.txt`, and reboot the app. This has not been tested.
