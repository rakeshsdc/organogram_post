...
def to_excel_bytes(pos: pd.DataFrame, extra: pd.DataFrame) -> bytes:
    ...
    buf = io.BytesIO()
    # person names are no longer entered; only write that column if a loaded file had names
    cols = [c for c in POS_COLS if c != "name" or (pos["name"] != "").any()]
    width_of = {"position_id": 12, "position_title": 32, "name": 22, "department": 20, "reports_to": 12,
                "status": 10, "email": 26, "phone": 16, "sort_order": 11}
    with pd.ExcelWriter(buf, engine="openpyxl") as writer:
        pos[cols].to_excel(writer, sheet_name="Positions", index=False)
        extra[EXTRA_COLS].to_excel(writer, sheet_name="Extra_Reporting", index=False)
        for name, widths in (
            ("Positions", [width_of[c] for c in cols]),
