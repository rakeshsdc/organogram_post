...
def _node_label(row: dict, notes: List[str], opts: ChartOptions) -> str:
    vacant = opts.highlight_vacant and (row["status"] == "Vacant" or not row["name"])
...
def _graph_title(title: str, colors: Dict[str, str], has_vacant: bool, has_extra: bool,
                 opts: ChartOptions) -> str:
    parts = []
    head = title or ""
    if opts.institution:
        head = f"{opts.institution}" + (f" - {title}" if title else "")
    if head:
        parts.append(f'<TR><TD><FONT POINT-SIZE="22"><B>{_e(head)}</B></FONT></TD></TR>')
    if opts.show_legend and (colors or has_vacant or has_extra):
        items = []
        if opts.color_by_department:
            for d, c in colors.items():
                items.append(f'<TD BGCOLOR="{c}" BORDER="1" COLOR="{BORDER_COLOR}"> {_e(d)} </TD>')
        if has_vacant and opts.highlight_vacant:
            items.append(f'<TD BORDER="1" COLOR="{VACANT_COLOR}"><FONT COLOR="{VACANT_COLOR}"> Vacant post </FONT></TD>')
        if has_extra:
            items.append(f'<TD BORDER="1" COLOR="{EXTRA_LINE_COLOR}"><FONT COLOR="{EXTRA_LINE_COLOR}"> - - - Additional reporting line </FONT></TD>')
        chunks = [items[i:i + 6] for i in range(0, len(items), 6)]
        legend_rows = "".join("<TR>" + "".join(ch) + "</TR>" for ch in chunks)
        if legend_rows:
            parts.append('<TR><TD><TABLE BORDER="0" CELLBORDER="0" CELLSPACING="3" CELLPADDING="2">'
                         + legend_rows + "</TABLE></TD></TR>")
    if not parts:
        return ""
    return '<<TABLE BORDER="0" CELLBORDER="0" CELLSPACING="6">' + "".join(parts) + "</TABLE>>"
...
def build_graph(pos: pd.DataFrame, extra: pd.DataFrame, *, roots: Optional[List[str]] = None,
    ...
    for pid in visible:  # dict keeps insertion order = tree order
        ...
        vacant = opts.highlight_vacant and (row["status"] == "Vacant" or not row["name"])
        has_vacant = has_vacant or vacant
    ...
    used = {by_id[p]["department"] for p in visible}
    page_colors = {d: c for d, c in colors.items() if d in used}
    label = _graph_title(title, page_colors if opts.color_by_department else {}, has_vacant,
                         has_extra, opts)
    if label:
