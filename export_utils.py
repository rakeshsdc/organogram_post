...
def build_word(pages: List[dict], pos: pd.DataFrame, extra: pd.DataFrame,
    ...
    if include_table and not pos.empty:
        ...
        cols = ["Level", "ID", "Position", "Name", "Department", "Status",
                "Reports to", "Also reports to", "Email", "Phone"]
        widths_cm = [1.5, 1.4, 5.0, 4.0, 3.2, 1.7, 4.2, 5.0, 4.3, 2.8]
        ...
        for pid, depth in tree_rows(pos):
            ...
            vals = [
                str(levels.get(pid, "")), pid,
                ("    " * depth) + row["position_title"],
                row["name"] or "-", row["department"], row["status"] or ("Filled" if row["name"] else "Vacant"),
                (f"{parent['position_id']} {parent['position_title']}" if parent else "-"),
                "; ".join(extras.get(pid, [])) or "-", row["email"], row["phone"],
            ]
            for c, v in enumerate(vals):
                _set_cell(cells[c], v, size=8,
                          color="C00000" if (c == 3 and v == "-" and row["status"] == "Vacant") else None)
